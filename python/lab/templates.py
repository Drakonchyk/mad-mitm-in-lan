...
class LabTemplateRenderer:
    ...
    @classmethod
    def from_repo_root(cls, repo_root: Path) -> "LabTemplateRenderer":
        ...
        return cls(
            ...
            attacker_ip=str(settings.attacker_ip),
            victim_ip=str(settings.victim_ip),
        )
    ...
    def render_zeek_policy_text(self) -> str:
        ...
        return (
            source.replace("__GATEWAY_IP__", self.config["GATEWAY_IP"])
            .replace("__DNS_SERVER__", self.config["DNS_SERVER"])
            .replace("__ATTACKER_IP__", self.config["ATTACKER_CIDR"].split("/", 1)[0])
            .replace("__VICTIM_IP__", self.config["VICTIM_CIDR"].split("/", 1)[0])
