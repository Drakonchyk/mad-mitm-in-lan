...
ATTACK_CMD="sleep 10; cd '${REMOTE_ROOT}' && exec timeout -s INT ${ATTACK_ACTIVE_SECONDS} env PYTHONPATH='./python' python3 -m mitm.cli --config ./lab.conf mitm-dns --interface vnic0 --enable-forwarding --domains iana.org"
AUX_CMD="sleep 45; ip neigh replace '${GATEWAY_IP}' lladdr '${GATEWAY_LAB_MAC}' nud permanent dev vnic0"
