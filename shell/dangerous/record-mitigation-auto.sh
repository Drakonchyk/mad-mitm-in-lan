...
confirm_danger_run "mitigation-recovery-auto"
start_lab_and_wait_for_access
prepare_attacker_research_workspace

ATTACK_CMD="sleep 10; cd '${REMOTE_ROOT}' && exec timeout -s INT ${ATTACK_ACTIVE_SECONDS} sudo python3 ./python/setup_all.py --config ./lab.conf mitm-dns --interface vnic0 --enable-forwarding --domains iana.org"
AUX_CMD="sleep 45; sudo ip neigh replace '${GATEWAY_IP}' lladdr '${GATEWAY_LAB_MAC}' nud permanent dev vnic0"

SKIP_LAB_START="1" \
ATTACK_JOB_HOST="attacker" \
ATTACK_JOB_LABEL="mitigation-recovery-auto" \
...
PLAN_SPOOFED_DOMAINS="iana.org" \
  "${REPO_ROOT}/shell/80-record-manual-scenario.sh" \
  "mitigation-recovery" \
  "${DURATION}" \
