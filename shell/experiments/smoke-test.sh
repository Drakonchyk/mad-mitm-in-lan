...
CAPTURE_PACKET_COUNT="${SMOKE_CAPTURE_PACKET_COUNT}" "${EXPERIMENT_SCRIPT_DIR}/run-baseline.sh"
...
run_short_recording \
  ...
  "smoke-arp-mitm-auto" \
  "cd '${REMOTE_ROOT}' && exec sudo env PYTHONPATH='./python' python3 -m mitm.cli --config ./lab.conf arp-poison --interface vnic0 --enable-forwarding"
...
run_short_recording \
  ...
  "smoke-arp-mitm-dns-auto" \
  "cd '${REMOTE_ROOT}' && exec sudo env PYTHONPATH='./python' python3 -m mitm.cli --config ./lab.conf mitm-dns --interface vnic0 --enable-forwarding"
