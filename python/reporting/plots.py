...
SEMANTIC_EVENT_COLORS = {
    ...
    "dhcp_reply_from_untrusted_switch_port_seen": "#14b8a6",
}
...
def _plot_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [row for row in rows if _is_plotted_scenario(str(row.get("scenario", "")))]
...
def _scenario_order(rows: list[dict[str, Any]]) -> list[str]:
    return [
        ...
        if _is_plotted_scenario(scenario) and rows_for_scenario(rows, scenario)
    ]
...
def _reliability_campaign_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        row
        for row in rows
        if str(row.get("scenario", "")).startswith("reliability-")
        and row.get("reliability_netem_loss_percent") is not None
    ]


def _plot_reliability_detection_survival(rows: list[dict[str, Any]], output_dir: Path, plt: Any) -> Path | None:
    ...
    for ax, scenario in zip(axes, scenarios):
        ...
        x_values = sorted({int(row.get("reliability_netem_loss_percent") or 0) for row in scenario_rows})
        for tool in TOOL_ORDER:
            y_values = []
            for loss_percent in x_values:
                matching = [row for row in scenario_rows if int(row.get("reliability_netem_loss_percent") or 0) == loss_percent]
        ...
        ax.set_title(f"{SCENARIO_LABELS.get(scenario, scenario)} Reliability Survival")
        ax.grid(True, axis="y", alpha=0.32)

    axes[-1].set_xlabel("NetEm packet loss (%)")
    axes[0].legend(loc="lower right")
    return _save(fig, output_dir / "figure-28-reliability-detection-survival.png", plt)


def _plot_reliability_alert_volume(rows: list[dict[str, Any]], output_dir: Path, plt: Any) -> Path | None:
    campaign_rows = _reliability_campaign_rows(rows)
    if not campaign_rows:
        return None
    ...
    return _save(fig, output_dir / "figure-29-reliability-alert-volume.png", plt)


def _plot_reliability_event_recall_triptych(rows: list[dict[str, Any]], output_dir: Path, plt: Any) -> Path | None:
    campaign_rows = _reliability_campaign_rows(rows)
    if not campaign_rows:
        return None
    ...
    fig.suptitle("Reliability Event Recall Dependency")
